...
### Dependencies
**OSS libraries only**: DuckDB, APScheduler, urllib3
...
### Probe Functions
Copied from `gapless-crypto-data/scratch/vision-futures-explorer/`:
- `historical_probe.py::check_symbol_availability()`: S3 HEAD request with error handling
- `futures_discovery.py::load_discovered_symbols()`: Load all 708 perpetual symbols

### Performance
**Single date**: 708 symbols in ~60 seconds (serial probing at 2 req/sec safe rate)
**Historical backfill**: ~2240 days × 708 symbols = 4-6 hours (with parallelization)
